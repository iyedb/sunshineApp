...
public class ForecastAdapter extends CursorAdapter {
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        return LayoutInflater.from(context).inflate(R.layout.list_item_forecast5, parent, false);
    }
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        tvHighTemp.setText(cursor.getString(ForecastFragment.COL_WEATHER_MAX_TEMP));

        // Read low temperature from cursor
        float low = cursor.getFloat(ForecastFragment.COL_WEATHER_MIN_TEMP);

        TextView tvLowTemp = (TextView) view.findViewById(R.id.list_item_low_textview);
        tvHighTemp.setText(cursor.getString(ForecastFragment.COL_WEATHER_MIN_TEMP));
    }
}